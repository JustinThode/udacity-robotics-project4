...
{
    //ROS_INFO("DriveToTargetRequest received - v:%1.2f, w:%1.2f", (float)req.linear_x, (float)req.angular_z);
    geometry_msgs::Twist motor_command;
    motor_command.linear.x = req.linear_x;
    motor_command.angular.z = req.angular_z;
    motor_command_publisher.publish(motor_command);
	
	res.msg_feedback = "Current speeds - vx: " + std::to_string(motor_command.linear.x) + " , wz: " + std::to_string(motor_command.angular.z);
